      ...
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 rounded-3xl p-6 sm:p-10 border border-slate-800 shadow-2xl relative overflow-hidden">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-3">
            ...
            <p className="text-xs text-slate-400">ডাটাবেচৰ পৰা তথ্য সংগ্ৰহ কৰা হৈছে...</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredData.map((item) => {
              const title = item.title_as || item.title || item.title_en || 'নিয়োগ বাৰ্তা';
              const dept = item.department_as || item.department || item.department_en || 'প্ৰশাসন';
              const desc = item.description_as || item.description || item.description_en || '';
              const badge = item.category_badge || item.categoryBadge || item.category || 'সাধাৰণ';
              const applyUrl = item.apply_url || item.applyUrl || '#';
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 bg-slate-950/80 border-t border-slate-800/60 flex items-center gap-2.5 backdrop-blur-sm">
                    ...
                    <button
                      type="button"
                      onClick={() => handleWhatsAppShare(title, dept, badge, deadline, applyUrl)}
                      title="হোৱাটছএপত শ্বেয়াৰ কৰক"
                      aria-label="হোৱাটছএপত শ্বেয়াৰ কৰক"
